 ...
 *   https://datatables.net/download/#bs/dt-1.10.16/cr-1.4.1/fc-3.2.3
 *
 * Included libraries:
 *   DataTables 1.10.16, ColReorder 1.4.1, FixedColumns 3.2.3
 */
...
(function( $, window, document, undefined ) {
	...
	function _fnAjaxParameters( settings )
	{
		...
		//add by yangxi (add param)
		param('pageType', settings.pageType);
	...
	DataTable.ext = _ext = {
		 ...
		 *  @type string
		 */
		build:"bs/dt-1.10.16/cr-1.4.1/fc-3.2.3",
...
DataTable.ext.renderer.pageButton.bootstrap = function ( settings, host, idx, buttons, page, pages ) {
	...
	try {
		// Because this approach is destroying and recreating the paging
		// elements, focus is lost on the  button which is bad for
		// accessibility. So we want to restore focus once the draw has
		// completed
		activeEl = $(host).find(document.activeElement).data('dt-idx');
	}
 ...
 *    be a jQuery collection, a jQuery or, DataTables API instance or
 *    settings object.
...
$.extend( $.fn.dataTable.defaults, {
	"language" : { // 国际化配置
		...
		"lengthMenu" : "每页 _MENU_ 条",/*页面显示数据条数选项*/
		"zeroRecords" : "没有找到数据",/*过滤后空的结果字符串*/
		"info" : "， 显示 _START_ 到  _END_ 条， 共 _TOTAL_ 条记录。",/*表格主要信息显示字符串*/
		"infoEmpty" : "记录数为0",/*当表格没有数据和搜索记录时，表格主要信息显示字符串*/
		"infoFiltered" : "(全部记录数 _MAX_ 条)",/*当使用搜索功能后，表格主要信息出追加的字符*/
		"infoPostFix" : "",/*追加到所有其他主要信息字符串之后*/
		"search" : "搜索",/*设置一个在Datatables初始化时的搜索选项*/
		"url" : "",/*从远程加载语言信息文件url*/
		"paginate" : {
	...
	/*"bJQueryUI": true,*/
	"dom": "<'#topPlugin'f>rt<'row'<'col-md-6 col-sm-6'li><'col-md-6 col-sm-6 pull-right'p>>", /*按什么顺序定义表的控制元素在页面上出现*/
	"bStateSave": true, // save datatable state(pagination, sort, etc) in cookie.
	"sort":false,　//默认不排序
	"paging" : true,// 分页按钮
	"searching" : false,// 搜索栏,前提是sDom必须有f（find）部分
	"lengthChange" : true,// 控制每行显示记录数
	"pageLength" : 10,// 初始默认每页显示行数
	"displayStart":0,/*初始化分页起点*/
	...
	"pagingType" : "full_numbers", // 分页，一共两种样式 另一种为two_button是datatables默认
	"processing" : true,//显示正在加载
	"serverSide" : true,//服务器分页
	"destroy": true,/*允许重新实例化Datatables*/
	/*"bSortCellsTop": true*/
	//"ordering": false,/*运行排序*/
	//"stripeClasses": [ 'gradeA odd',  'gradeA even']/*在表中设置斑马条纹类的行*/
} );
